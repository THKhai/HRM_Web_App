...
for /L %%i in (1,1,%MAX_ATTEMPTS%) do (
    ...
    REM Check Postgres
    docker exec -i Postgres_container psql -U postgres -d AuthData -c "SELECT 1" >nul 2>&1
    if !errorlevel! neq 0 (
...
for %%f in (.\Repository\Migration_scripts\postgres\*.sql) do (
    echo Executing %%~nxf...
    @echo off
    docker exec -i Postgres_container psql -U postgres -d AuthData -f /migrations/%%~nxf
    if !errorlevel! neq 0 (
